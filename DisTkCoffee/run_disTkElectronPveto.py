...
from disTkElectronPveto_core import (
    empty_counts,
...
def validate_file_schema(files: list[str], tree: str) -> None:
    ...
    lines = ["Input file is not compatible with the DisTkCoffee electron Pveto schema."]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the OSUNano custom NanoAOD electron Pveto analysis."
    )
    parser.add_argument("--single-electron", nargs="+", required=True)
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Apply the extra 2022 E/F/G ECAL water-leak veto used by Table 15.",
    )
    ...
    payload = make_payload(files, args.tree, results, configuration)
